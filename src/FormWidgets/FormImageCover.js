...
class ImgArea extends React.Component {  
  ...
  render () {
    ...
    var popups = this.props.value.popups.map((popup,index) =>
      <div className="item" key={index} id={'item'+index} name={'item'+index} draggable="true" onDragStart={this.drag_start} style={styles['item'+index]}>
    ...
    return (
      <div className="row">
        <div className="col-12">
          ...
          <div className='gridPopup' id="imgcontain" onDrop={this.drop} onDragOver={this.drag_over}>
            {popups}
          </div>
        </div>
      </div>
